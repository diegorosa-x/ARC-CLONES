...
const Cart: React.FC<CartProps> = ({ isOpen, onClose, items, onRemove }) => {
  const total = items.reduce(
    (sum, item) => sum + (item.finalPrice || item.basePrice) * item.quantity,
    0,
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-hidden">
      <div
        className="absolute inset-0 bg-slate-900/80 dark:bg-brand-bg/90 backdrop-blur-sm"
        onClick={onClose}
      />
      <div className="absolute inset-y-0 right-0 max-w-full flex">
        <div className="w-screen max-w-md bg-brand-bg shadow-2xl flex flex-col border-l border-brand-muted transition-colors duration-300">
          <div className="px-8 py-10 border-b border-brand-muted flex items-center justify-between bg-brand-bg">
            <div>
              <h2 className="text-xl font-brand font-bold text-slate-900 dark:text-white uppercase tracking-widest">
                Carrinho
              </h2>
          ...
          <div className="flex-1 py-8 overflow-y-auto px-8 hide-scrollbar bg-brand-bg">
            {items.length === 0 ? (
              <div className="text-center py-24">
                ...
                <p className="text-slate-500 uppercase tracking-widest text-[11px] font-bold">
                  Nenhum clone na sua bag.
                </p>
                <button
                  onClick={onClose}
                  className="mt-8 text-[11px] font-bold text-brand-gold underline uppercase tracking-widest"
                >
                  Explorar Coleção
                </button>
              </div>
              ...
              <ul className="space-y-10">
                {items.map((item) => (
                  <li
                    key={`${item.id}-${item.selectedFactory}`}
                    className="flex animate-in slide-in-from-right-4 duration-300"
                  >
                    <div className="relative h-28 w-24 flex-shrink-0 overflow-hidden border border-brand-muted bg-slate-100 dark:bg-slate-900 rounded-sm">
                      <Image
                        src={item.image}
                        alt={item.name}
                        fill
          ...
          <div className="border-t border-brand-muted px-8 py-10 bg-brand-bg/50">
            ...
            <p className="mt-0.5 text-[10px] text-slate-500 uppercase tracking-[0.15em] mb-10 text-center">
              Frete Expresso Grátis Incluso
            </p>
