...
import { getShoppingAdvice } from "../services/geminiService"; // <-- GARANTA que existe lá

interface AIShopperProps {
  onRecommendationClick: (ids: string[]) => void;
}

type ChatMsg = { role: "user" | "ai"; text: string; ids?: string[] };

const AIShopper: React.FC<AIShopperProps> = ({ onRecommendationClick }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await getShoppingAdvice(userText);
      ...
      setHistory((prev) => [
        ...
        {
          role: "ai",
          text: "Desculpe, o sistema está temporariamente fora do ar. Tente novamente em alguns minutos.",
        },
      ]);
    } finally {
      setLoading(false);
      ...
      {isOpen && (
        <div className="fixed bottom-28 right-10 z-50 w-[340px] md:w-[420px] bg-brand-bg border border-brand-muted shadow-2xl flex flex-col rounded-sm overflow-hidden animate-in slide-in-from-bottom-5 duration-500">
          ...
          <div className="h-96 overflow-y-auto p-6 space-y-6 text-[12px] bg-brand-bg flex flex-col hide-scrollbar">
            {history.length === 0 && (
              <div className="text-slate-500 dark:text-slate-600 text-center py-16 flex flex-col items-center">
                ...
                <p className="italic font-light leading-relaxed">
                  &quot;Quero um modelo esportivo preto&quot;
                  <br />
                  ou &quot;Qual a diferença entre X e Y?&quot;
                </p>
              </div>
            )}
            ...
            {history.map((msg, i) => (
              <div
                ...
                className={`${
                  msg.role === "user"
                    ? "self-end bg-brand-muted text-slate-900 dark:text-white"
                    : "self-start bg-brand-bg border border-brand-muted text-slate-700 dark:text-slate-300"
                } max-w-[85%] p-4 rounded-sm shadow-sm animate-in fade-in duration-300`}
              >
          ...
          <div className="p-4 border-t border-brand-muted flex items-center space-x-4 bg-brand-bg">
            ...
            <button
              onClick={handleSend}
              className="text-brand-gold hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-40"
              disabled={loading}
              aria-label="Enviar"
            >
